...
?.third_digit
	LDA !ram_hex2dec_third_digit
	ORA.w #<color> : STA !HUD_EXTRAS_BUFFER+14, X
endmacro

macro draw_two(color)
	LDA !ram_hex2dec_second_digit : AND #$00FF
	CMP.w #1
?templabel:
	; match cycles
?.second_digit
...
.roomtime
	...
	LDA !lowram_draw_tmp2 : %hex_to_dec_fast() : %draw_all_two(!yellow)
	DEX #4 ; to make it match new position
	LDA !lowram_draw_tmp : %hex_to_dec_fast() : %draw_three(!white)
...
.idletime
	...
	LDA !lowram_idle_frames_copy : %hex_to_dec_fast() : %draw_three(!white)

.segmenttime
	LDA !ram_counters_segment : %update_counter_line()

	LDA !lowram_seg_frames : %hex_to_dec_fast() : %draw_all_two(!gray)
	DEX #4
	LDA !lowram_seg_seconds : %hex_to_dec_fast() : %draw_all_two(!yellow)
	DEX #4
	LDA !lowram_seg_minutes : %hex_to_dec_fast() : %draw_three(!white)

.coordinates
	LDA !ram_xy_toggle : %update_counter_line()
	LDA $20 : %set_coords() : %draw_all_three(!yellow)
	DEX #6
	LDA $22 : %set_coords() : %draw_all_three(!white)

.subpixels
	LDA !ram_subpixels_toggle : %update_counter_line()
	LDA $2A : %set_2_coords() : %draw_all_two(!yellow)
	DEX #4
	LDA $2B : %set_2_coords() : %draw_all_two(!white) 
